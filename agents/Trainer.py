...
class Trainer(object):
    ...
    def create_agent_to_agent_group_dictionary(self):
        ...
        agent_to_agent_group_dictionary = {
            ...
            "DQN Vanila": "DQN_Agents",
            "DQN Fixed Vanila": "DQN_Agents",
        ...
        for agent_number, agent_class in enumerate(self.agents):
            ...
            for i in range(3):
                if self.config.visualise_overall_agent_results:
                    agent_rolling_score_results = [results[0][i] for results in self.results[agent_name]]
                    self.visualise_overall_agent_results(agent_rolling_score_results, agent_name + i, show_mean_and_std_range=True)
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            agent_results.append([game_scores , rolling_scores, len(game_scores[0]), -1 * max(game_scores[0]), time_taken])
